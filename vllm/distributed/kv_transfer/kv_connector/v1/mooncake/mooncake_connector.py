    ...
    async def _build_transfer_params(
        ...
        for d_req_id, send_meta in ready_reqs:
            ...
            for group_index, (local_group, remote_group) in enumerate(
                ...
                # Both lists now cover the same token blocks, index for index.
                # A NULL_BLOCK_ID entry means that position carries no
                # transferable state, so drop the pair rather than either side
                # alone: filtering the two lists independently would shift one
                # against the other and pair blocks from different positions.
                kept = [
                    (local_id, remote_id)
